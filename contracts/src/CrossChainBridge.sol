...
contract CrossChainBridge {
     ...
     * @param recipient The address to receive funds on the destination chain.
     */
    function sendCrossChainDeposit(uint256 destinationChainId, address asset, uint256 amount, address recipient) external {
        // 1. Lock/Burn assets on this chain
        IERC20(asset).transferFrom(msg.sender, address(this), amount);
        
        // 2. Emit event for CCTP Relayer
        // We encode 'recipient' instead of 'msg.sender' as the beneficiary
        bytes memory payload = abi.encode(recipient, asset, amount, "DEPOSIT");
